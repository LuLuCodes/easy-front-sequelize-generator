...
export const transformerFactory = (
  transformCase: TransformCase | TransformMap
): TransformFn => {
  ...
  return function (value: string, target: TransformTarget) {
    if (!isASCII(value)) {
      console.warn(
        `Unsupported case transformation for non ASCII characters:`,
        value
      );
      return value;
    }
...
export const caseTransformer = (
  ...
  const transformed: ITableMetadata = {
    ...
    paranoid: tableMetadata.paranoid,
    aliasFields: tableMetadata.aliasFields,
    columns: {},
  ...
  for (const [columnName, columnMetadata] of Object.entries(
    ...
    transformed.columns[columnName] = Object.assign({}, columnMetadata, {
 ...
 */
export const generatePrecisionSignature = (
