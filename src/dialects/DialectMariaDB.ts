...
import {
  generatePrecisionSignature,
  warnUnknownMappingForDataType,
} from './utils';
    ...
    for (const column of columns) {
      ...
      const columnMetadata: IColumnMetadata = {
        ...
        ...(this.mapDbTypeToSequelize(column.DATA_TYPE) && {
          dataType:
            'DataType.' +
            this.mapDbTypeToSequelize(column.DATA_TYPE).key.split(' ')[0], // avoids 'DOUBLE PRECISION' key to include PRECISION in the mapping
        }),
      ...
      switch (column.DATA_TYPE) {
        ...
        case 'float':
        case 'double':
          columnMetadata.dataType += generatePrecisionSignature(
